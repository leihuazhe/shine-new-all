package com.maple.shine.versions.java17;
...
public class SwitchWithPatternMatching {
    ...
    static String stringify(Object value) {
        switch (value) {
            ...
            default:
                return "Is untested type =(: " + value;
        }
    }

    /**
     * Guarded patterns
     * only support in Java 17. guarded patterns
     */
    ...
    static long calculateAssetPrice(Asset asset) {
        switch (asset) {
            case Stock s:
                return s.calculatePrice();
            // guard
            case Bond b when b.isExpired(): throw new IllegalStateException("The bond is expired");
            case Bond b:
                return b.calculatePrice();
            case CertificateOfDeposit cd when cd.isExpired():throw new IllegalStateException("The CD is expired");
            case CertificateOfDeposit cd:
                return cd.calculatePrice();
            case null:
                throw new IllegalArgumentException("Invalid asset");
...
final class CertificateOfDeposit extends Asset {
    ...
    public LocalDate calculateMaturityDate() {
        switch (term) {
            case SIX_MONTH:
                return issuedDate.plusMonths(6);
            case ONE_YEAR:
                return issuedDate.plusYears(1);
            case EIGHTEEN_MONTH:
                return issuedDate.plusMonths(18);
            case TWO_YEAR:
                return issuedDate.plusYears(2);
            default:
